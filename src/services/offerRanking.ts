...
export type InstagramServiceType = 'Seguidores' | 'Curtidas' | 'Visualizacoes' | 'Comentarios' | 'Metricas' | 'Outros';
...
function getText(service: Pick<GuidedInstagramService, 'category' | 'name' | 'description'>): string {
  return normalizeText(`${service.category} ${service.name} ${service.description ?? ''}`);
}

function getPricePerThousand(service: GuidedInstagramService): number {
  return Number(service.final_price);
}

function getMinAmount(service: GuidedInstagramService): number {
...
function getQualitySignals(service: GuidedInstagramService): number {
  const text = getText(service);
  let score = 0;

  if (service.raw_payload?.refill) score += 18;
  if (containsAny(text, ['premium', 'ultra', 'high quality', 'qualidade'])) score += 14;
  if (containsAny(text, ['organico', 'organic', 'reais', 'real'])) score += 10;
  if (containsAny(text, ['garantia', 'warranty', 'refill', 'reposicao'])) score += 8;
  if (containsAny(text, ['instant', 'rapido', 'fast'])) score += 4;

  return score;
}

export function inferInstagramServiceType(service: Pick<GuidedInstagramService, 'category' | 'name' | 'description'>): InstagramServiceType {
  const text = getText(service);

  if (containsAny(text, ['seguidores', 'followers'])) return 'Seguidores';
  if (containsAny(text, ['curtidas', 'likes'])) return 'Curtidas';
  if (containsAny(text, ['visualiz', 'views', 'view'])) return 'Visualizacoes';
  if (containsAny(text, ['coment', 'comments'])) return 'Comentarios';
  if (containsAny(text, ['salvamentos', 'compartilhamentos', 'impressoes', 'metric'])) return 'Metricas';

  return 'Outros';
}

export function inferOriginFromService(service: Pick<GuidedInstagramService, 'category' | 'name' | 'description'>): 'Brasileiro' | 'Internacional' | 'Misto' | 'Todos' {
  const text = getText(service);

  if (containsAny(text, ['brasileir', ' brasil ', ' br '])) return 'Brasileiro';
  if (containsAny(text, ['internacional', 'global', 'mundial', 'worldwide'])) return 'Internacional';
  if (containsAny(text, ['misto', 'mix', 'mista'])) return 'Misto';

  return 'Todos';
}

export function rankInstagramOffers(
  ...
  const analyzed = services.map((service) => {
    ...
    if (context.origin === 'Brasileiro' && containsAny(text, ['brasileir', ' brasil ', ' br '])) score += 10;
    if (context.origin === 'Internacional' && containsAny(text, ['internacional', 'global', 'mundial'])) score += 10;
    if (context.origin === 'Misto' && containsAny(text, ['misto', 'mix'])) score += 8;
    if (context.type === 'Seguidores' && containsAny(text, ['drop', 'queda'])) score -= 12;
...
export function getServiceTrustSignals(service: GuidedInstagramService): string[] {
  const signals = [
...
export function getUpsellSuggestions(service: Pick<GuidedInstagramService, 'category' | 'name' | 'description'>): UpsellSuggestion[] {
