...
export async function createOrderActionLog(input: OrderActionLogInput): Promise<void> {
  await pool.query(
    `insert into order_action_logs (
       telegram_id,
       order_id,
       action,
       supplier_order_id,
...
export async function countRecentOrderActionLogs(
  telegramId: number,
  seconds: number,
  action?: OrderCareAction,
  orderId?: number
  ...
  const values: Array<number | string> = [telegramId, seconds];
