...
import { getGuidedPlatformOptions, getTypeOptionsByPlatform, type GuidedOption } from '../utils/guidedCatalog.js';
...
export type UpsellButton = {
  token: string;
  label: string;
};

export type SupportTicketButton = {
  id: number;
  status: string;
};

export type OrderCareAction = 'status' | 'refill' | 'cancel';

function upperButtonLabel(label: string): string {
  return label.toUpperCase();
}

function decorateGuidedOption(label: string): string {
...
export function guidedTypesInline(platform: string) {
  const options = getTypeOptionsByPlatform(platform);
  const rows = [];

  for (let index = 0; index < options.length; index += 2) {
    const pair = options.slice(index, index + 2).map((option) => (
      Markup.button.callback(upperButtonLabel(decorateGuidedOption(option.label)), `gtype:${option.value}`)
    ));
    rows.push(pair);
  }
...
export function guidedSimpleOptionsInline(prefix: string, options: Array<SimpleOption | GuidedOption>, backAction: string) {
...
export function guidedServiceOptionsInline(services: Array<{ id: number; label: string }>, backAction: string) {
  const buttons = services.map((service) => [Markup.button.callback(upperButtonLabel(service.label), `gsvc:${service.id}`)]);
  buttons.push([Markup.button.callback('⬅️ VOLTAR', backAction)]);
  return Markup.inlineKeyboard(buttons);
}

export function guidedPackageOptionsInline(packages: Array<{ quantity: number; amountCents: number; label: string }>, backAction: string) {
...
export function guidedPaymentOptionsInline(hasEnoughBalance: boolean) {
  const buttons = [];
...
export function orderCareEntryInline(
  action: OrderCareAction,
  orders: Array<{ id: number; service_name?: string | null }>
) {
...
export function orderCarePromptInline(action: OrderCareAction) {
  return Markup.inlineKeyboard([
...
export function orderCareResultInline(
  orderId: number,
  options?: {
    canRefill?: boolean;
    canCancel?: boolean;
...
export function supportHubInline(options?: { hasTickets?: boolean; hasContact?: boolean }) {
  const rows = [
...
export function supportOrderSelectionInline(orders: Array<{ id: number }>) {
...
export function supportTicketsInline(tickets: SupportTicketButton[]) {
...
export function supportTicketDetailInline(ticketId: number, closed: boolean) {
  const rows = [];
...
export function supportAdminTicketInline(ticketId: number, closed: boolean) {
  const rows = [];
...
export function categoriesInline(categories: string[]) {
  const buttons = categories.map((category) => [
    Markup.button.callback(
      upperButtonLabel(category.length > 20 ? `${category.substring(0, 17)}...` : category),
...
export function servicesInline(services: Array<{ id: number; name: string; final_price: string }>, category: string) {
...
export function serviceBuyInline(serviceId: number, category: string) {
  const categoryToken = getCategoryToken(category);
  return Markup.inlineKeyboard([
...
export function guidedSummaryInline(linkLabel: string) {
  return Markup.inlineKeyboard([
...
export function recentOrdersInline(orders: Array<{ id: number }>) {
...
export function upsellOptionsInline(options: UpsellButton[]) {
