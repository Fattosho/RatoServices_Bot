import { GuidedInstagramService, type ManagedOrderRecord } from '../db/repositories.js';
import type { CheckoutCartRecord, OrderTrackingRecord, UserOrderDetails } from '../db/commerce.js';
import type { OrderCareAction } from '../db/orderCare.js';
import type { SupportMessageRecord, SupportTicketRecord } from '../db/support.js';
import type { RankedOffer, UpsellSuggestion } from '../services/offerRanking.js';
...
function isFollowersService(text: string): boolean {
  const normalized = normalizeText(text);
  return normalized.includes('seguidores') || normalized.includes('followers');
}

function isLikesService(text: string): boolean {
  const normalized = normalizeText(text);
  return normalized.includes('curtidas') || normalized.includes('likes');
}

function isInstagramService(text: string): boolean {
  const normalized = normalizeText(text);
...
function getCommercialQuantityNoun(type?: string, platform?: string): string {
...
function formatCommercialQuantity(value: number, type?: string, platform?: string): string {
  return `${formatQuantity(value)} ${getCommercialQuantityNoun(type, platform)}`;
}

function getReadableTypeLabel(platform: string, type: string): string {
  return getTypeDisplayLabel(platform, type);
}

function isFixedPackageService(service: Pick<GuidedInstagramService, 'raw_payload'>): boolean {
...
function cleanCommercialName(name: string): string {
  return name
    .replace(/\[[^\]]+\]/g, '')
    .replace(/\(\s*queda[^)]*\)/gi, '')
    .replace(/\bS\d+\b/gi, '')
...
export function getCommercialServiceName(name: string): string {
  return cleanCommercialName(name);
}

function getCommercialBadge(service: GuidedInstagramService): string {
  ...
  if (text.includes('premium')) return `Premium | ${refill}`;
  if (text.includes('ultra')) return `Ultra | ${refill}`;
  if (text.includes('organ')) return `Organico | ${refill}`;
  if (text.includes('reais')) return `Perfis reais | ${refill}`;

  return refill;
}

function getAudienceLabel(service: GuidedInstagramService): string {
...
function getServiceHook(service: GuidedInstagramService): string {
  const text = normalizeText(`${service.category} ${service.name} ${service.description ?? ''}`);
...
function getDescriptionPreview(service: GuidedInstagramService): string {
...
export function getBotGuideMessage(): string {
  return [
...
export function getGuidedPlatformPrompt(): string {
  return [
    '📱 Escolha a plataforma',
    '',
...
export function getPlatformTypePrompt(platform: string): string {
  return [
...
export function getInstagramTypePrompt(): string {
  return getPlatformTypePrompt('Instagram');
}

export function getVariantPrompt(platform: string, type: string): string {
...
export function getOriginPrompt(type: string): string {
  return getVariantPrompt('Instagram', type);
}

export function getRefillPrompt(platform: string, type: string, variant?: string): string {
  const label = getVariantLabel(platform, type);
  const typeLabel = getReadableTypeLabel(platform, type);
  return [
...
export function getGuidedServiceCatalogMessage(
  type: string,
  origin: string,
  refillLabel: string,
...
export function getServicePresentation(service: ServiceLike): string {
  return [
...
export function getGuidedServiceButtonLabel(service: GuidedInstagramService): string {
...
export function getGuidedServiceSummary(service: GuidedInstagramService, selection?: GuidedSelectionSummary): string {
  const min = getNumeric(service.raw_payload?.min, 0);
...
export function getMandatoryInstructionsMessage(
  service: ServiceLike,
  selection: Pick<GuidedSelectionSummary, 'platform' | 'type'> & {
    quantity?: number;
    amount?: number;
    link?: string;
...
export function getCheckoutLinkPrompt(
  service: ServiceLike,
  selection?: Pick<GuidedSelectionSummary, 'platform' | 'type'>
): string {
...
export function getInvalidLinkMessage(type?: string, platform = 'Instagram'): string {
...
export function getPackagePrompt(service: GuidedInstagramService, selection?: GuidedSelectionSummary, _note?: string): string {
  const fixedPackage = isFixedPackageService(service);
...
export function getBalancePurchaseSummary(
...
export function getWalletPrompt(): string {
  return [
    '💳 Recarregar saldo',
    '',
...
export function getWalletSummary(balance: string, _totalAdded: string, _totalSpent: string): string {
  return [
    '💰 Minha carteira',
    '',
...
export function getWalletHubMessage(balance: string, _totalAdded: string, _totalSpent: string): string {
  return [
    '💰 SALDO DA SUA CONTA',
    '',
    `💵 SALDO DISPONIVEL: R$ ${balance}`,
...
export function getSupportMessage(contact?: string): string {
  if (contact) {
    return [
      '🆘 Suporte',
      '',
...
function formatQuantityCompact(value: number | null): string {
  if (!value) return 'Nao informado';
  return formatQuantity(value);
}

function formatTargetPreview(platform: string | null | undefined, link: string | null | undefined): string {
...
function formatServiceDisplayName(name?: string | null): string {
  if (!name) return 'Servico';
  return cleanCommercialName(name);
}

function getSupportCategoryLabel(category: string): string {
  if (category === 'pedido') return 'Pedido';
  if (category === 'pagamento') return 'Pagamento';
  if (category === 'saldo') return 'Saldo';
  return 'Outro assunto';
}

function getSupportStatusLabel(status: string): string {
  if (status === 'waiting_support') return 'aguardando suporte';
  if (status === 'waiting_customer') return 'aguardando voce';
  if (status === 'closed') return 'encerrado';
  return status;
}

function formatSupportMessageLine(message: SupportMessageRecord): string {
...
export function getSupportHubMessage(contact?: string): string {
  return [
...
export function getSupportCategoryPrompt(): string {
  return [
    '🎫 Abrir atendimento',
    '',
...
export function getSupportOrderPrompt(category: string): string {
  return [
    '📦 Vincular pedido',
    '',
...
export function getSupportDescriptionPrompt(category: string, orderId?: number): string {
  return [
...
export function getSupportTicketCreatedMessage(ticket: SupportTicketRecord): string {
  return [
...
export function getSupportTicketsListMessage(tickets: SupportTicketRecord[]): string {
...
export function getSupportTicketDetailMessage(ticket: SupportTicketRecord, messages: SupportMessageRecord[]): string {
  return [
    `🎫 Ticket #${ticket.id}`,
    '',
    `📂 Assunto: ${getSupportCategoryLabel(ticket.category)}`,
...
export function getSupportReplyPrompt(ticket: SupportTicketRecord): string {
  return [
...
export function getSupportReplyConfirmation(ticketId: number): string {
  return [
...
export function getSupportAdminNotificationMessage(ticket: SupportTicketRecord, firstMessage?: string): string {
  return [
...
export function getSupportAdminReplyPrompt(ticket: SupportTicketRecord): string {
  return [
...
export function getSupportAdminReplySentMessage(ticketId: number): string {
  return `✅ Resposta enviada no ticket #${ticketId}.`;
}

export function getSupportAdminTicketClosedMessage(ticketId: number): string {
  return `✅ Ticket #${ticketId} encerrado.`;
}

export function getSupportClosedForCustomerMessage(ticketId: number): string {
  return [
...
export function getSupportReplyFromTeamMessage(ticketId: number, reply: string): string {
  return [
...
export function getSupportGroupSetupMessage(
  chatId: string,
  options?: {
    title?: string;
...
export function getSupportCloseUsageMessage(): string {
  return [
...
export function getCartRecoveryMessage(cart: CheckoutCartRecord): string {
...
export function getRepeatOrderPrompt(order: UserOrderDetails): string {
  return [
    '🔁 Comprar novamente',
    '',
...
function getOrderCareActionLabel(action: OrderCareAction): string {
  if (action === 'refill') return 'refil';
  if (action === 'cancel') return 'cancelamento';
  return 'status';
}

function getManualSupplierStatusLabel(status: string): string {
  ...
  if (status === 'submitted') return '📨 Em processamento';
  return `📍 ${status}`;
}

export function getOrderCareEntryMessage(action: OrderCareAction, hasOrders: boolean): string {
...
export function getOrderCareManualPrompt(action: OrderCareAction): string {
  return [
...
export function getOrderCareOrderNotFoundMessage(orderId: number): string {
  return [
...
export function getOrderCareRateLimitMessage(): string {
  return [
...
export function getManualOrderStatusMessage(
  order: ManagedOrderRecord,
  details: {
    supplierStatusLabel: string;
    supplierStatusRaw?: string;
    remains?: string;
    startCount?: string;
    canRefill?: boolean;
    canCancel?: boolean;
...
export function getFallbackOrderStatusMessage(order: ManagedOrderRecord): string {
  return [
    `📦 Pedido #${order.id}`,
    '',
    `🛍️ Servico: ${formatServiceDisplayName(order.service_name)}`,
...
export function getRefillSuccessMessage(order: ManagedOrderRecord, supplierMessage: string): string {
  return [
...
export function getRefillBlockedMessage(order: ManagedOrderRecord, reason: string): string {
  return [
...
export function getCancelSuccessMessage(order: ManagedOrderRecord, supplierMessage: string): string {
  return [
...
export function getCancelBlockedMessage(order: ManagedOrderRecord, reason: string): string {
  return [
...
export function getReadableSupplierStatusLabel(status: string): string {
  return getManualSupplierStatusLabel(status);
}

export function getOrderStatusUpdateMessage(
...
export function getUpsellMessage(
  service: Pick<GuidedInstagramService, 'category' | 'name' | 'description'>,
