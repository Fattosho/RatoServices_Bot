...
export type RefillMode = 'with_refill' | 'without_refill' | 'any';
export type GuidedVariantKind = 'origin' | 'subtype' | 'none';
...
function getSearchText(service: ServiceLike): string {
...
function isAnyProfileHandle(value: string): boolean {
  return /^@?[a-zA-Z0-9._-]{2,60}$/.test(value);
}

export function getGuidedPlatformOptions(): GuidedOption[] {
...
export function getCatalogPlatformsForMenu(platform: string): string[] {
  if (platform === 'Outros') {
...
export function getTypeDisplayLabel(platform: string, type: string): string {
...
export function getTypeOptionsByPlatform(platform: string): GuidedOption[] {
  const storeTypes = getStoreTypesByPlatform(platform as CatalogPlatform);
...
export function getVariantStep(platform: string, type: string): {
  kind: GuidedVariantKind;
  title: string;
...
export function getVariantLabel(platform: string, type: string): string {
  const step = getVariantStep(platform, type);
  if (step.kind === 'origin') return 'Origem';
  if (step.kind === 'subtype') return 'Subtipo';
  return 'Filtro';
}

export function getRefillOptions(): GuidedOption[] {
  return [
...
export function getRefillLabel(value: RefillMode): string {
  if (value === 'with_refill') return 'Com reposicao';
  if (value === 'without_refill') return 'Sem reposicao';
  return 'Qualquer';
}

export function inferServiceType(platform: string, service: ServiceLike): string {
...
function typeMatches(text: string, terms: string[]): boolean {
  return hasAny(text, terms);
}

function isBrazilian(text: string): boolean {
...
function matchesVariantByType(text: string, platform: string, type: string, variant: string): boolean {
  if (variant === 'Outros') {
    return true;
  }

  if (type === 'Seguidores' || type === 'Curtidas') {
    if (variant === 'Brasileiro') return isBrazilian(text);
    if (variant === 'Internacional') return isInternational(text);
    if (variant === 'Misto') return isMixed(text) || (isBrazilian(text) && isInternational(text));
    return true;
  }

  if (type === 'Visualizacoes') {
    if (platform === 'Telegram') {
      if (variant === 'Outros') return true;
      return hasAny(text, ['postagem', 'postagens', 'post', 'views']);
    }

    if (variant === 'Reels') return hasAny(text, ['reel', 'reels']);
    if (variant === 'Stories') return hasAny(text, ['story', 'stories']);
    if (variant === 'Lives') return hasAny(text, ['live', 'ao vivo']);
    if (variant === 'Videos') return hasAny(text, ['video', 'videos', 'views', 'visualizacoes']);
    if (variant === 'Posts') return hasAny(text, ['post', 'postagem', 'publicacao']);
    if (variant === 'Shorts') return hasAny(text, ['shorts', 'short']);
    if (variant === 'Canal') return hasAny(text, ['canal', 'channel']);
    if (variant === 'Perfil') return hasAny(text, ['perfil', 'profile']);
    return platform !== 'Instagram';
  }

  if (type === 'Comentarios') {
    if (variant === 'Brasileiros') return isBrazilian(text);
    if (variant === 'Internacionais') return isInternational(text);
    if (variant === 'Personalizados') return hasAny(text, ['personalizado', 'custom']);
    return true;
  }

  if (type === 'Metricas') {
    if (variant === 'Salvamentos') return hasAny(text, ['salvamentos', 'saves']);
    if (variant === 'Compartilhamentos') return hasAny(text, ['compartilh', 'shares']);
    if (variant === 'Impressoes') return hasAny(text, ['impress', 'impressions']);
    if (variant === 'Alcance') return hasAny(text, ['alcance', 'reach']);
    if (variant === 'Visitas') return hasAny(text, ['visitas', 'visit', 'profile visits']);
    return true;
  }

  if (type === 'Reacoes') {
    if (variant === 'Aleatorio') return hasAny(text, ['aleatorio', 'aleatório']);
    if (variant === 'Curtir') return hasAny(text, ['[👍]', '👍']);
    if (variant === 'Coracao') return hasAny(text, ['[❤️]', '[❤]', '❤️', '❤']);
    if (variant === 'Risada') return hasAny(text, ['[😂]', '😂']);
    if (variant === 'Surpreso') return hasAny(text, ['[😲]', '😲']);
    if (variant === 'Triste') return hasAny(text, ['[😥]', '😥']);
    if (variant === 'Oracao') return hasAny(text, ['[🙏]', '🙏']);
    return true;
  }
...
function getRefillSignalLines(service: ServiceLike): string[] {
  const rawText = [
    service.name,
    service.description,
    service.raw_payload?.description
...
export function serviceHasRefillSignal(service: ServiceLike): boolean {
...
export function matchesGuidedFilters(
  service: ServiceLike,
  platform: string,
  type: string,
  ...
  if (refillMode !== 'any') {
    ...
    if (refillMode === 'with_refill' && !hasRefill) return false;
    if (refillMode === 'without_refill' && hasRefill) return false;
  }

  return true;
}

export function isProfileBasedRequest(platform: string, type: string): boolean {
  if (type === 'Seguidores') {
    return true;
  }
...
export function getMandatoryInstructions(platform: string, type: string): string[] {
...
export function getLinkTargetLabel(platform: string, type: string): string {
  ...
  if (platform === 'Telegram' && type === 'Visualizacoes') return 'link da postagem';
  if (platform === 'Telegram' && type === 'Membros') return 'link do grupo ou canal';
  if (platform === 'WhatsApp' && (type === 'Seguidores' || type === 'Reacoes')) return 'link do canal';
  if (platform === 'Facebook' && type === 'Curtidas + Seguidores') return 'link da pagina';
  if (isProfileBasedRequest(platform, type)) return 'perfil, pagina ou canal';
  if (platform === 'YouTube') return 'video, short, live ou canal';
  if (platform === 'TikTok') return 'perfil ou video';
  if (platform === 'Facebook') return 'pagina, perfil, post, reel ou video';
  return 'destino do servico';
}

export function normalizeGuidedCheckoutLink(platform: string, type: string, rawInput: string): string | null {
