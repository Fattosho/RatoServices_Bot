...
function getSupportChatIdCandidates(preferredChatId) {
...
function getMigratedSupportChatId(error) {
    if (!error || typeof error !== 'object') {
        return undefined;
    }
    const migrated = error.response?.parameters?.migrate_to_chat_id;
...
async function notifyAffiliateLeadStart(ctx, referredByCode, lead) {
    if (!referredByCode) {
        return;
    }
    ...
    try {
        await ctx.telegram.sendMessage(affiliateOwner.telegram_id, [
...
function getQuantityNoun(type, platform) {
...
function getQuantityStep(type, platform) {
    if (platform === 'Instagram')
        return 100;
    if (type === 'Comentarios')
        return 50;
...
function buildPackages(service, type) {
    ...
    const budgetStartQuantity = roundUpToStep(Math.max(min, Math.ceil(MIN_COMMERCIAL_PACKAGE_AMOUNT / Math.max(unitPrice, 0.000001))), quantityStep);
    ...
    if (!packages.length) {
        const minimumBudgetQuantity = roundUpToStep(Math.max(min, Math.ceil(MIN_COMMERCIAL_PACKAGE_AMOUNT / unitPrice)), quantityStep);
...
function formatOrderTarget(platform, link) {
    if (!link)
...
function calculateRefillUpsellAmount(baseAmount, apiDifference) {
...
async function resolveRefillUpsell(purchase, baseService) {
    ...
    const refillServices = await getGuidedServices(purchase.platform, purchase.type, purchase.origin, 'with_refill');
    if (!refillServices.length) {
        return null;
    }
    const [refillService] = sortGuidedServices(purchase.platform, purchase.type, purchase.origin, refillServices.filter((service) => service.id !== baseService.id));
    if (!refillService) {
        return null;
    }
    const apiDifference = Math.max(0, Number((((Number(refillService.final_price) - Number(baseService.final_price)) / 1000) * purchase.quantity).toFixed(2)));
    return {
        service: refillService,
        extraAmount: calculateRefillUpsellAmount(purchase.amount, apiDifference)
    };
}
async function openWalletHome(ctx) {
    const senderId = ctx.from?.id;
    if (!senderId)
        return;
    const wallet = await getUserWallet(senderId);
    await ctx.reply(getWalletHubMessage(wallet?.balance ?? '0.00', wallet?.total_added ?? '0.00', wallet?.total_spent ?? '0.00'), topupAmountsInline());
}
async function openProfileHome(ctx) {
    await ctx.reply([
        '👤 PERFIL',
        '',
...
function getCartStage(purchase) {
    if (!purchase?.platform)
        return 'platform';
    if (!purchase.type)
        return 'type';
    if (getVariantStep(purchase.platform, purchase.type).kind !== 'none' && !purchase.origin)
        return 'origin';
    if (!purchase.quantity || !purchase.amount)
        return 'package';
...
async function syncPurchase(ctx, purchase) {
...
async function finalizePurchaseSession(ctx, status, convertedOrderId) {
    const senderId = ctx.from?.id;
...
function sortGuidedServices(platform, type, variant, services) {
    if (platform === 'Instagram') {
...
async function resolveBestGuidedService(purchase, requestedRefillMode) {
    ...
    for (const attempt of attempts) {
        const services = await getGuidedServices(purchase.platform, purchase.type, attempt.variant, attempt.refillMode);
        if (!services.length) {
            continue;
        }
...
async function openPackageStep(ctx, purchase, requestedRefillMode, editCurrentMessage = true, backAction = 'guided_back_variant') {
    if (!purchase.platform || !purchase.type) {
...
async function continueGuidedPurchaseFlow(ctx, purchase, editCurrentMessage = true) {
    if (!purchase.platform || !purchase.type) {
        await ctx.reply(getGuidedPlatformPrompt(), guidedPlatformsInline());
        return;
    }
    await openPackageStep(ctx, purchase, 'without_refill', editCurrentMessage, getVariantStep(purchase.platform, purchase.type).kind === 'none'
        ? 'guided_back_type'
        : 'guided_back_variant');
}
async function showSelectedPackageSummary(ctx, service, purchase, editCurrentMessage = true) {
    if (!purchase.platform || !purchase.type || !purchase.quantity || !purchase.amount) {
        return;
    }
...
function isSupportTeamContext(ctx) {
...
async function openSupportHome(ctx, editCurrentMessage = false) {
    const senderId = ctx.from?.id;
...
async function notifySupportTeam(bot, ticket, messageText) {
    if (!env.supportChatId && !resolvedSupportChatId) {
        return;
    }
    try {
        const sent = await callSupportChatApi(bot, 'sendMessage', {
...
function extractTicketIdFromSupportThreadMessage(message) {
...
function resetOrderCareSession(ctx) {
    ctx.session.orderCare = undefined;
}
function canAttemptRefill(orderStatus) {
    return ['completed', 'partial'].includes(orderStatus);
}
function canAttemptCancel(orderStatus) {
    return ['pending', 'processing', 'submitted'].includes(orderStatus);
}
async function hitOrderActionRateLimit(telegramId, action, orderId) {
    ...
    if (typeof orderId === 'number') {
        const sameOrderAttempts = await countRecentOrderActionLogs(telegramId, ORDER_ACTION_SAME_ORDER_WINDOW_SECONDS, action, orderId);
...
async function openOrderCareEntry(ctx, action, editCurrentMessage = false) {
    const senderId = ctx.from?.id;
    if (!senderId) {
        return;
    }
...
async function handleManualOrderAction(ctx, action, orderId) {
    const senderId = ctx.from?.id;
    if (!senderId) {
        return;
    }
    ...
    if (action === 'status') {
        ...
        await ctx.reply(getManualOrderStatusMessage(order, {
    ...
    if (action === 'refill') {
        if (!supplierOrderId) {
            ...
            await ctx.reply(getRefillBlockedMessage(order, 'Esse pedido ainda nao entrou na etapa de processamento necessaria para solicitar refil.'), orderCareResultInline(order.id));
            return;
        }
        if (!order.service_raw_payload?.refill) {
            ...
            await ctx.reply(getRefillBlockedMessage(order, 'Essa oferta foi cadastrada sem suporte a refil.'), orderCareResultInline(order.id, {
        ...
        if (details) {
            ...
            if (!canAttemptRefill(details.normalizedStatus)) {
                ...
                await ctx.reply(getRefillBlockedMessage(order, `No momento o pedido esta como ${getReadableSupplierStatusLabel(details.normalizedStatus)}. O refil costuma fazer sentido depois que a entrega fecha ou fica parcial.`), orderCareResultInline(order.id, {
    ...
    if (order.status === 'pending' && !supplierOrderId) {
        ...
        await ctx.reply(getCancelSuccessMessage(order, 'O pedido estava pendente no bot e foi cancelado antes de iniciar o processamento.'), orderCareResultInline(order.id));
        return;
    }
    if (!supplierOrderId) {
        await createOrderActionLog({
        ...
        await ctx.reply(getCancelBlockedMessage(order, 'Ainda nao existe uma etapa de processamento ativa para este pedido.'), orderCareResultInline(order.id));
        return;
    }
    const details = await fetchSupplierOrderStatus(supplierOrderId);
    if (details) {
        ...
        if (!canAttemptCancel(details.normalizedStatus)) {
            ...
            await ctx.reply(getCancelBlockedMessage(order, `No momento o pedido esta como ${getReadableSupplierStatusLabel(details.normalizedStatus)} e nao parece elegivel para cancelamento.`), orderCareResultInline(order.id, {
...
async function sendRecentOrders(ctx) {
    const senderId = ctx.from?.id;
    if (!senderId)
        return;
    const orders = await listRecentOrders(senderId);
...
async function sendUpsellSuggestions(ctx, service) {
    ...
    await ctx.reply(getUpsellMessage(service, suggestions), upsellOptionsInline(suggestions.map((item) => ({
        token: item.token,
        label: item.label
    }))));
}
async function presentCheckoutOptions(ctx, service, purchase) {
    const senderId = ctx.from?.id;
    ...
    await ctx.reply(getBalancePurchaseSummary(service, nextPurchase.platform, nextPurchase.type, nextPurchase.quantity, nextPurchase.amount, nextPurchase.link, balanceText, hasEnoughBalance), guidedPaymentOptionsInline(hasEnoughBalance));
}
async function sendTopupSuggestion(ctx) {
    const senderId = ctx.from?.id;
...
async function sendTopupPix(ctx, amount) {
    const senderId = ctx.from?.id;
    if (!senderId) {
        ...
        try {
            ...
            if (status === 'approved' || status === 'paid') {
                ...
                if (purchase?.serviceId && purchase.link && purchase.quantity && purchase.amount && currentBalance >= purchase.amount) {
                    const service = await getServiceById(purchase.serviceId);
                    if (service) {
...
async function sendOrderPix(ctx) {
    ...
    const referredBy = await getUserReferrerCode(senderId);
    const orderId = await createOrder(senderId, purchase.serviceId, orderAmount, orderLink, orderQuantity, {
...
async function resumeCart(ctx) {
    ...
    if (variantStep.kind !== 'none' && !purchase.origin) {
        await ctx.reply(getVariantPrompt(purchase.platform, purchase.type), guidedSimpleOptionsInline('gorigin', variantStep.options, 'guided_back_type'));
        return;
    }
...
async function openUpsellFlow(ctx, type) {
    const purchase = {
        platform: 'Instagram',
        type
    };
    ...
    await ctx.reply(`✨ Abrindo ofertas de ${type}. Primeiro escolha ${variantStep.kind === 'origin' ? 'a origem' : 'o subtipo'} para receber recomendacoes mais precisas.`, guidedSimpleOptionsInline('gorigin', variantStep.options, 'guided_back_type'));
}
export function registerBotHandlers(bot) {
    ...
    bot.command('tickets', async (ctx) => {
        ...
        for (const ticket of tickets) {
            await ctx.reply(getSupportAdminNotificationMessage(ticket, ticket.last_message_preview ?? ''), supportAdminTicketInline(ticket.id, ticket.status === 'closed'));
        }
    });
    bot.command('chatid', async (ctx) => {
        ...
        await ctx.reply(getSupportGroupSetupMessage(chatId, {
            title,
    ...
    bot.action(/support_category:(.+)/, async (ctx) => {
        ...
        if (recentOrders.length) {
            ...
            await ctx.answerCbQuery();
            await ctx.editMessageText(getSupportOrderPrompt(category), supportOrderSelectionInline(recentOrders.slice(0, 5).map((order) => ({ id: order.id }))));
            return;
        }
        ctx.session.support = {
            mode: 'awaiting_description',
    ...
    bot.action(/support_order:(.+)/, async (ctx) => {
        ...
        await ctx.answerCbQuery();
        await ctx.editMessageText(getSupportDescriptionPrompt(support.category, orderId), supportAbortInline());
    });
    bot.action('support_my_tickets', async (ctx) => {
        ...
        if (!tickets.length) {
            await ctx.answerCbQuery();
            await ctx.editMessageText('📭 Voce ainda nao possui tickets abertos ou recentes.', supportHubInline({ hasContact: Boolean(env.supportContact) }));
            return;
        }
        await ctx.answerCbQuery();
        await ctx.editMessageText(getSupportTicketsListMessage(tickets), supportTicketsInline(tickets.map((ticket) => ({
            id: ticket.id,
    ...
    bot.action(/gtype:(.+)/, async (ctx) => {
        ...
        await ctx.editMessageText(getVariantPrompt(nextPurchase.platform, type), guidedSimpleOptionsInline('gorigin', variantStep.options, 'guided_back_type'));
        await ctx.answerCbQuery();
    });
    ...
    bot.action('guided_back_variant', async (ctx) => {
        ...
        await ctx.editMessageText(getVariantPrompt(purchase.platform, purchase.type), guidedSimpleOptionsInline('gorigin', variantStep.options, 'guided_back_type'));
        await ctx.answerCbQuery();
    });
    bot.action('refill_upsell_accept', async (ctx) => {
        ...
        const refillService = await getServiceById(purchase.refillUpsellServiceId);
        if (!refillService) {
    ...
    bot.action('guided_back_package', async (ctx) => {
        ...
        await ctx.answerCbQuery();
        await openPackageStep(ctx, purchase, 'without_refill', true, getVariantStep(purchase.platform, purchase.type).kind === 'none'
            ? 'guided_back_type'
            : 'guided_back_variant');
    });
    bot.action('guided_instructions', async (ctx) => {
        ...
        if (!purchase.link) {
            ...
            await ctx.answerCbQuery();
            await ctx.reply(getCheckoutLinkPrompt(service, {
                platform: purchase.platform,
                type: purchase.type
    ...
    bot.action('confirm_balance_purchase', async (ctx) => {
        ...
        const debited = await debitUserWallet(senderId, purchase.amount, 'order', String(purchase.serviceId), `Compra do servico ${getCommercialServiceName(service.name)}`);
        if (!debited) {
            await ctx.answerCbQuery();
            await presentCheckoutOptions(ctx, service, purchase);
            return;
        ...
        const referredBy = await getUserReferrerCode(senderId);
        const orderId = await createOrder(senderId, purchase.serviceId, purchase.amount, purchase.link, purchase.quantity, {
            displayServiceName: getCommercialServiceName(service.name),
    ...
    bot.on('text', async (ctx, next) => {
        ...
        if (supportSession && senderId) {
            ...
            if (supportSession.mode === 'awaiting_description' && supportSession.category) {
                ...
                if (fullTicket) {
                    await ctx.reply(getSupportTicketCreatedMessage(fullTicket), supportTicketDetailInline(fullTicket.id, false));
        ...
        if (purchase.step === 'link') {
            const normalizedLink = normalizeGuidedCheckoutLink(purchase.platform ?? 'Instagram', purchase.type ?? 'Outros', ctx.message.text);
            if (!normalizedLink) {
