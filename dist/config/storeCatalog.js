...
const STORE_SERVICE_MAP = new Map(STORE_SERVICE_CATALOG.map((entry) => [entry.externalId, entry]));
export function getStoreServiceEntry(externalId) {
    return STORE_SERVICE_MAP.get(externalId);
}
export function isStoreServiceAllowed(externalId) {
    return STORE_SERVICE_MAP.has(externalId);
}
export function getStorePlatforms() {
    return [...new Set(STORE_SERVICE_CATALOG.map((entry) => entry.platform))];
}
export function getStoreTypesByPlatform(platform) {
    const types = [...new Set(STORE_SERVICE_CATALOG
