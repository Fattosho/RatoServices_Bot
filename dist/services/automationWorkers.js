...
import { listOrdersForStatusSync, listRecoverableCheckoutCarts, markCheckoutCartReminderSent, markOrderStatusNotified, touchOrderSupplierCheck } from '../db/commerce.js';
import { fetchSupplierOrderStatus } from '../integrations/supplierApi.js';
async function runCartRecovery(bot) {
    const carts = await listRecoverableCheckoutCarts();
    for (const cart of carts) {
        await bot.telegram.sendMessage(cart.telegram_id, getCartRecoveryMessage(cart), cartRecoveryInline());
        await markCheckoutCartReminderSent(cart.telegram_id);
    }
}
async function runOrderTracking(bot) {
    ...
    for (const order of orders) {
        ...
        await bot.telegram.sendMessage(order.telegram_id, getOrderStatusUpdateMessage(order, supplierStatus), supplierStatus.normalizedStatus === 'completed'
            ? recentOrdersInline([{ id: order.id }])
            : undefined);
        await markOrderStatusNotified(order.id, supplierStatus.normalizedStatus);
    }
}
export function startAutomationWorkers(bot) {
