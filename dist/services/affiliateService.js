...
export async function getAffiliatePanel(telegramId) {
...
export async function processOrderCommission(orderId) {
    ...
    if (commissionAmount > 0) {
        await addAffiliateCredits(order.referred_by_code, commissionAmount, 'order', String(orderId), `Credito de indicacao do pedido #${orderId}`);
    }
}
export async function processTopupCommission(topupId) {
    ...
    if (commissionAmount > 0) {
        await addAffiliateCredits(topup.referred_by_code, commissionAmount, 'topup', String(topupId), `Credito de indicacao da recarga #${topupId}`);
    }
}
