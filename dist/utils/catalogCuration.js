...
export function detectCatalogPlatform(category, name, description) {
    ...
    if (normalizedCategory.startsWith('ig') || containsAny(text, ['instagram']))
        return 'Instagram';
    if (normalizedCategory.startsWith('tk') || containsAny(text, ['tiktok']))
        return 'TikTok';
    if (normalizedCategory.startsWith('yt') || containsAny(text, ['youtube']))
        return 'YouTube';
    if (normalizedCategory.startsWith('fb') || containsAny(text, ['facebook']))
        return 'Facebook';
    if (containsAny(text, ['telegram']))
        return 'Telegram';
    if (containsAny(text, ['whatsapp']))
        return 'WhatsApp';
    if (containsAny(text, ['twitter', 'x/twitter']))
        return 'X';
    if (containsAny(text, ['kwai']))
        return 'Kwai';
    if (containsAny(text, ['kick']))
        return 'Kick';
    if (containsAny(text, ['snackvideo']))
        return 'SnackVideo';
    if (containsAny(text, ['google maps', 'google']))
        return 'Google';
    if (containsAny(text, ['reddit']))
        return 'Reddit';
    return 'Outros';
}
export function curateCatalogEntry(entry) {
    ...
    if (containsAny(text, ['premium', 'organico', 'reais', 'estavel']))
        score += 10;
    if (containsAny(text, ['reposicao', ' refill ', 'r30', 'r60', 'r90', 'r365']))
        score += 8;
    if (containsAny(text, ['instaneo', 'instantaneo', 'rapido', 'hq']))
        score += 4;
    if (containsAny(text, ['[ pacote ]', '1 mes', '30 dias']))
        score -= 3;
    return {
        active: true,
        platform,
